...
Any application that connects to the protocol, a calling app, an email client, a fintech integration, registers with the foundation and receives a unique application identifier (UUID). This identifier is included in every interaction with the resolver network. It enables per-application consent preferences, attribution of reputation data to its source, revenue sharing with applications that contribute data, and enforcement of protocol rules through UUID revocation. Registration requires a verifiable business identity but no content review. Applications that contribute reputation data participate in a revenue-sharing programme funded by lookup fees.
